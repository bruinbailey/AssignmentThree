...
router.route('/assignments/class/:className/:assignmentName')
	...
	.put(function(req, res){
		if(req.params.className !== req.body.class || 
				req.params.assignmentName !== req.body.name) {
					 res.status(400);
					 res.json("class or assignment info does not match")
				 }
		Assign.findOne({class: req.body.className,
	...
	.delete(function(req, res) {
		Assign.deleteOne({class: req.params.className,
			name: req.params.assignmentName})
			.exec()
			.catch(err => {
			...
			.then(doc => {
				console.log("if i get here something is happening with just the res")
				res.status(204);
			})
	});
